...
const PlaceOrderScreen = () => {
    ...
    updatedCart.itemsPrice = addDecimals(cart.cartItems.reduce((acc, item) => acc + item.price * item.qty, 0))
    ...
    updatedCart.totalPrice= Number(
        updatedCart.itemsPrice) + Number(updatedCart.shippingPrice) + Number(updatedCart.taxPrice
    )

    // read order create reducer
    const orderCreate = useSelector((state) => state.orderCreate)
    const {order, success, error} = orderCreate

    // if order was created before
    useEffect(() => {
        if(success){
